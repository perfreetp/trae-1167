  ...
  targets.forEach(function (mold) {
    ...
    if (nextDate <= endDate) {
      const exists = plans.find(function (p) {
        return p.moldCode === mold.code && p.planDate === nextDate && (p.status === 'pending' || p.status === 'overdue');
      });
      if (!exists) {
        plans.push({
...
function handleDone(id, opts) {
  ...
  writeJson(MAINTENANCE_FILE, plans);
  addLog('maintain:done', '完成保养: 模具' + plans[idx].moldCode + ', 计划ID:' + planId, plans[idx].moldCode);
...
function handleList(opts) {
  ...
  const table = new Table({
    head: [chalk.cyan('ID'), chalk.cyan('模具编号'), chalk.cyan('计划日期'), chalk.cyan('类型'), chalk.cyan('状态'), chalk.cyan('逾期天数'), chalk.cyan('备注')],
    colWidths: [14, 14, 12, 10, 10, 10, 20],
    style: { 'padding-left': 1, 'padding-right': 1 }
  });

  var todayStr = today();
  plans.forEach(function (p) {
    var isOverdue = (p.status === 'pending' || p.status === 'overdue') && p.planDate < todayStr;
    var statusStr;
    if (p.status === 'done') {
      statusStr = chalk.green('已完成');
    } else if (isOverdue || p.status === 'overdue') {
      statusStr = chalk.red('逾期');
    } else {
      statusStr = chalk.yellow('待保养');
    }
    var overdueDays = '';
    if (isOverdue || p.status === 'overdue') {
      if (p.status === 'done') {
        ...
        overdueDays = chalk.red(dayjs().diff(dayjs(p.planDate), 'day') + '天');
      }
    }
    table.push([p.id, p.moldCode, p.planDate, p.type || '-', statusStr, overdueDays, p.remark || '-']);
  });
