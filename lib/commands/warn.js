...
exports.handler = function (opts) {
  ...
  if (showMaintenance) {
    ...
    maintenanceCount = overduePlans.length + noPlanOverdue.length;

    if (overduePlans.length > 0 || noPlanOverdue.length > 0) {
      ...
      console.log(chalk.gray('共 ' + overduePlans.length + ' 条逾期计划, ' + noPlanOverdue.length + ' 套逾期未安排保养模具'));
    } else {
