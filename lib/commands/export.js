...
function buildReportCSV(type, exportOpts) {
  ...
  switch (type) {
    ...
    case 'report-flow':
      ...
      var flowEvents = flowModule.reportFlow.buildData(flowCode, { from: exportOpts && exportOpts.from, to: exportOpts && exportOpts.to });
      if (!flowEvents || flowEvents.length === 0) return null;
      return flowEvents.map(function (e) {
        return { date: e.time, action: e.label, detail: e.detail };
      });
...
exports.handler = function (opts) {
  ...
  if (isReport) {
    if (opts.type === 'report-flow' && !opts.flowCode) {
      console.error(chalk.red('导出流转明细请使用 --flow-code <模具编号>'));
      process.exit(1);
    }
