...
exports.handler = function (opts) {
  ...
  if (isReport) {
    var reportRows = buildReportCSV(opts.type);
    if (!reportRows) {
  ...
  switch (opts.type) {
    ...
    default:
      ...
      console.log(chalk.gray('原始数据: molds, maintenance, repairs, logs'));
      console.log(chalk.gray('报表数据: report-utilization, report-repair-rate, report-idle, report-summary'));
      process.exit(1);
  }
