...
function buildFlowData(code, opts) {
  ...
  if (statusFilter) {
    var statusCodes = new Set(
      molds.filter(function (m) { return m.status === statusFilter; })
        .map(function (m) { return m.code.toLowerCase(); })
    );
    if (targetCodes) {
      var intersection = new Set();
      targetCodes.forEach(function (c) { if (statusCodes.has(c)) intersection.add(c); });
      targetCodes = intersection;
    } else {
      targetCodes = statusCodes;
    }
  }
  ...
  logs.forEach(function (l) {
    ...
    if (l.action === 'out' || l.action === 'in' || l.action === 'maintain:plan' || l.action === 'repair') return;
