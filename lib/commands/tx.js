...
function handleEdit(id, opts) {
  ...
  if (opts.returnDate && tx.status === 'open') {
    ...
    var moldsForStatus = readJson(MOLDS_FILE) || [];
    var moldIdxForStatus = moldsForStatus.findIndex(function (m) { return m.code === tx.moldCode; });
    if (moldIdxForStatus !== -1 && moldsForStatus[moldIdxForStatus].status === 'out_stock') {
      moldsForStatus[moldIdxForStatus].status = 'in_stock';
      moldsForStatus[moldIdxForStatus].lastOutPerson = '';
      moldsForStatus[moldIdxForStatus].updatedAt = new Date().toISOString();
      writeJson(MOLDS_FILE, moldsForStatus);
      changes.push({ field: 'moldStatus', label: '模具状态', oldVal: 'out_stock', newVal: 'in_stock' });
    }
  }
...
function handleAdd(code, opts) {
  ...
  if (usageCount > 0) {
    var moldIdx = molds.findIndex(function (m) { return m.code === code; });
    if (moldIdx !== -1) {
      molds[moldIdx].usedCount += usageCount;
      molds[moldIdx].updatedAt = new Date().toISOString();
      writeJson(MOLDS_FILE, molds);
    }
  }

  addLog('tx:add', '补录流转: ' + code + ', 出库: ' + outDate + (returnDate ? ', 归还: ' + returnDate : '') + ', 领用人: ' + person + (usageCount > 0 ? ', 模次: ' + usageCount : ''), code);
